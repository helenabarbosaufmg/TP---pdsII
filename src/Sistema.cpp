...
bool Sistema::hospedeCadastrado(int codigo) const {
    for (const Hospede& h : hospedes) {
        if (h.HCodigo() == codigo) return true;
    }
    return false;
}
...
int Sistema::posicaoHospede(int codigo) const {
    for (std::size_t i = 0; i < hospedes.size(); ++i) {
        if (hospedes[i].HCodigo() == codigo) return static_cast<int>(i);
    }
    return -1;
}
...
void Sistema::cadastrarHospede(Hospede h) {
    ...
    for (char c : h.HRG()) if (!isdigit(c)) { cout << "Erro: RG deve conter apenas numeros." << endl; return; }
    for (char c : h.HTelefone()) if (!isdigit(c)) { cout << "Erro: telefone deve conter apenas numeros." << endl; return; }
...
const Hospede* Sistema::buscarHospedePorCodigo(int codigo) const {
    for (const Hospede& h : hospedes) if (h.HCodigo() == codigo) return &h;
    return nullptr;
}
...
bool Sistema::quartoCadastrado(int numero) const {
    for (const Quartos& q : quartos) if (q.QNumero() == numero) return true;
    return false;
}

int Sistema::posicaoQuarto(int numero) const {
    for (std::size_t i = 0; i < quartos.size(); ++i) if (quartos[i].QNumero() == numero) return static_cast<int>(i);
    return -1;
}

void Sistema::cadastrarQuarto(Quartos q) {
    if (quartoCadastrado(q.QNumero())) { cout << "Erro: ja existe quarto com esse numero." << endl; return; }
    string tipo = q.QTipo(); for (char& c : tipo) c = toupper(c);
    if (tipo != "SOLTEIRO" && tipo != "CASAL" && tipo != "LUXO") { cout << "Erro: tipo de quarto invalido. Use 'Solteiro', 'Casal' ou 'Luxo'." << endl; return; }
    if (q.QPreco() <= 0) { cout << "Erro: preco do quarto deve ser um valor positivo." << endl; return; }
    quartos.push_back(q);
...
const Quartos* Sistema::buscarQuartoPorNumero(int numero) const {
    for (const Quartos& q : quartos) if (q.QNumero() == numero) return &q;
    return nullptr;
}

void Sistema::listarStatusQuartos() const {
    cout << "\nStatus de Reservas (Lista de Quartos)\n";
    if (quartos.empty()) { cout << "Nenhum quarto cadastrado.\n"; return; }
...
void Sistema::ocuparQuarto(int numeroQuarto) {
    ...
    if (pos == -1) { cout << "Quarto nao encontrado.\n"; return; }
    if (quartos[pos].estaOcupado()) { cout << "Quarto " << numeroQuarto << " ja esta OCUPADO.\n"; return; }
    quartos[pos].ocupar();
...
void Sistema::liberarQuarto(int numeroQuarto) {
    int pos = posicaoQuarto(numeroQuarto);
    if (pos == -1) { cout << "Quarto nao encontrado.\n"; return; }
    if (!quartos[pos].estaOcupado()) { cout << "Quarto " << numeroQuarto << " ja esta DISPONIVEL.\n"; return; }
    quartos[pos].liberar();
...
// Reservas
// -------------------------------------------------
bool Sistema::reservaSobreposta(int numeroQuarto, const string& checkIn, const string& checkOut) const {
    for (const Reserva& r : reservas) {
        if (r.RQuarto().QNumero() == numeroQuarto) {
            ...
            if (!(checkOut <= inExistente || checkIn >= outExistente)) return true;
        }
    }
    return false;
}
...
void Sistema::criarReserva(Reserva r) {
    ...
    if (!hospedeCadastrado(codigoHospede)) { cout << "Erro: hospede nao esta cadastrado." << endl; return; }

    int numeroQuarto = r.RQuarto().QNumero();
    int posQuarto = posicaoQuarto(numeroQuarto);
    if (posQuarto == -1) { cout << "Erro: quarto nao esta cadastrado." << endl; return; }

    string checkIn = r.RCheckIn();
    string checkOut = r.RCheckOut();
    if (checkIn.size() != 10 || checkOut.size() != 10 || checkIn[2] != '/' || checkIn[5] != '/' || checkOut[2] != '/' || checkOut[5] != '/') {
        cout << "Erro: formato de data invalido (use dd/mm/aaaa)." << endl; return;
    }

    if (reservaSobreposta(numeroQuarto, checkIn, checkOut)) { cout << "Erro: ja existe reserva nesse periodo para este quarto." << endl; return; }
...
const Reserva* Sistema::buscarReservaPorNumero(int numeroReserva) const {
    for (const Reserva& r : reservas) if (r.RNumeroReserva() == numeroReserva) return &r;
    return nullptr;
}

void Sistema::mostrarHospedePorReserva(int numeroReserva) const {
    const Reserva* r = buscarReservaPorNumero(numeroReserva);
    if (!r) { cout << "Hospede nao encontrado para este numero de reserva." << endl; return; }
...
void Sistema::mostrarDadosReservaCompleta(int numeroReserva) const {
    const Reserva* r = buscarReservaPorNumero(numeroReserva);
    if (!r) { cout << "Reserva nao encontrada." << endl; return; }
    cout << "\nDados da Reserva\n";
    cout << "Numero da reserva: " << r->RNumeroReserva() << endl;
...
void Sistema::fazerCheckIn(int numeroReserva) {
    for (Reserva& r : reservas) {
        if (r.RNumeroReserva() == numeroReserva) { r.setStatus("Em andamento"); cout << "Check-in realizado com sucesso." << endl; return; }
    }
...
void Sistema::fazerCheckOut(int numeroReserva) {
    for (Reserva& r : reservas) {
        if (r.RNumeroReserva() == numeroReserva) {
            ...
            int numeroQuarto = r.RQuarto().QNumero();
            int posQuarto = posicaoQuarto(numeroQuarto);
            if (posQuarto != -1) quartos[posQuarto].liberar();
